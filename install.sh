...
_() {
	...
	downloadAndUnpackDockerfiles() {
		...
		curl -#L --progress-bar "https://github.com/opla/community-edition/archive/$rev.zip" -o $tempDir/source.zip
		cd $tempDir
		unzip -q source.zip
		rm source.zip
		mv community-edition* opla-ce
	...
	test_opla_up() {
		until wget -q http://front.localhost/ && echo "Connected to Opla Frontend !"; do
			sleep 5
		done
	}
